            ...
            <div class="card-body">
              <form method="POST" action="{{ route('teacher.register.post') }}">
                ...
                <div class="form-check form-check-info text-start mb-3">
                  <input class="form-check-input" type="checkbox" name="terms" id="flexCheckDefault" {{ old('terms') ? 'checked' : '' }} required>
                ...
                <p class="text-sm mt-3 mb-0">Already have an account? <a href="{{ route('teacher.login') }}" class="text-dark font-weight-bolder">Sign in</a></p>
              </form>
              {{-- عرض الأخطاء العامة --}}
              @if($errors->any())
