      ...
      <div class="container-fluid py-1 px-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
            ...
            <li class="breadcrumb-item text-sm"><a class="opacity-5 text-dark" href="{{ route('teacher.exams.show', $exam) }}">{{ $exam->title }}</a></li>
            <li class="breadcrumb-item text-sm text-dark active" aria-current="page">Grade Submission</li>
          </ol>
          <h6 class="font-weight-bolder mb-0">Grade Exam Submission</h6>
        </nav>
      </div>
    </nav>
    <!-- End Header -->
          ...
          <div class="card mb-4">
            <div class="card-header pb-0">
              <div class="d-flex justify-content-between align-items-center">
                ...
                <div>
                  <a href="{{ route('teacher.exams.show', $exam) }}" class="btn btn-secondary btn-sm">
                  ...
                  <div class="row text-center">
                    ...
                    <div class="col-3">
                      ...
                      @if($submission->score !== null)
                        <span class="text-sm font-weight-bold text-success">{{ $submission->score }}/{{ $exam->total_grade }}</span>
                      @else
                        <span class="text-sm text-muted">Not graded</span>
                      @endif
                    </div>
                    <div class="col-3">
                      <h6 class="text-sm font-weight-bold mb-0">Started</h6>
                      <span class="text-sm">{{ $submission->started_at ? $submission->started_at->format('M d, H:i') : 'N/A' }}</span>
                    </div>
                    <div class="col-3">
          ...
          <div class="row">
            ...
            <div class="col-lg-4">
              <div class="card">
                ...
                <div class="card-body">
                  <form action="{{ route('teacher.grading.update-exam', [$exam, $submission]) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3">
                      ...
                      <div class="input-group">
                        <input type="number" step="0.01" class="form-control" id="score" name="score" 
                               ...
                               min="0" max="{{ $exam->total_grade }}" required>
                        <span class="input-group-text">/ {{ $exam->total_grade }}</span>
                      </div>
                      <small class="text-muted">Enter score out of {{ $exam->total_grade }}</small>
                    </div>

                    <div class="mb-3">
                      <label for="status" class="form-label">Status</label>
                      <select class="form-select" id="status" name="status" required>
                        <option value="started" {{ old('status', $submission->status) == 'started' ? 'selected' : '' }}>Started</option>
              ...
              <div class="card mt-3">
                ...
                <div class="card-body">
                  ...
                  <div class="mb-2">
                    ...
                    <p class="mb-0">{{ ucfirst($exam->delivery_type) }}</p>
                  </div>
                  <div class="mb-2">
                  ...
                  <div class="mb-2">
                    ...
                    <p class="mb-0">{{ $exam->duration ?? 'N/A' }} minutes</p>
                  </div>
                  <div class="mb-0">
                    <small class="text-muted">Total Grade:</small>
                    <p class="mb-0">{{ $exam->total_grade }}</p>
                  </div>
                </div>
              </div>
              ...
              <div class="card mt-3">
                ...
                <div class="card-body">
                  <div class="mb-2">
                    ...
                    <p class="mb-0">{{ $submission->started_at ? $submission->started_at->format('M d, Y H:i:s') : 'N/A' }}</p>
                  </div>
                  <div class="mb-2">
                    <small class="text-muted">Submitted:</small>
                    <p class="mb-0">{{ $submission->submitted_at ? $submission->submitted_at->format('M d, Y H:i:s') : 'N/A' }}</p>
                  </div>
                  @if($submission->started_at && $submission->submitted_at)
                    <div class="mb-0">
                      <small class="text-muted">Time Taken:</small>
                      <p class="mb-0">{{ $submission->started_at->diffForHumans($submission->submitted_at, true) }}</p>
                    </div>
                  @endif
                </div>
              </div>
      ...
      <div class="row mt-4">
        <div class="col-12">
          @include('Teacher.grading.comments-section', [
            'commentRoute' => route('teacher.grading.add-exam-comment', [$exam, $submission])
          ])
        </div>
      </div>
    </div>
