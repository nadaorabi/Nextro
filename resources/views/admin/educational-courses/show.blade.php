        ...
        <div class="container-fluid py-4">
                ...
                <div class="col-md-3">
                    <div class="card stats-card success">
                        <div class="card-body p-3 text-center">
                            ...
                            <h2>{{ $course->courseInstructors->count() ?? 0 }}</h2>
                        </div>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="card stats-card info">
                        <div class="card-body p-3 text-center">
                            ...
                            <h2>{{ $course->materials->count() ?? 0 }}</h2>
                        </div>
                    </div>
                </div>
                <div class="col-md-3">
                                            ...
                                            @else
                                                ...
                                                @if($course->hasDiscount())
                                                    <br><small class="text-muted">
                                                        Original: {{ $course->formatted_original_price }}
                                                    </small>
                                                @endif
                                            @endif
                                        </div>
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <div class="info-value">{{ $course->currency ?? 'USD' }}</div>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <div class="info-value">
                                            @if($course->discount_percentage > 0)
                                                <span class="badge bg-warning">{{ $course->discount_percentage }}%</span>
                                            @else
            ...
            <div class="card info-card mb-4">
                <div class="card-body">
                    ...
                    @if($course->courseInstructors->count() > 0)
                        <div class="row">
                            @foreach($course->courseInstructors as $instructor)
                                <div class="col-md-6 col-lg-4 mb-3">
                                    <div class="list-item">
                                        <div class="d-flex align-items-center">
                                            <div class="flex-shrink-0">
                                                <img src="{{ asset('images/default-avatar.png') }}" 
                                                    alt="Instructor" class="avatar-sm">
                                            </div>
                                            <div class="flex-grow-1 ms-3">
                                                <h6 class="mb-1">{{ $instructor->user->name ?? 'N/A' }}</h6>
                                                <p class="mb-1 text-muted small">{{ $instructor->user->email ?? 'N/A' }}</p>
                                                <span class="badge bg-info">{{ $instructor->role ?? 'Instructor' }}</span>
                                            </div>
                                            <div class="flex-shrink-0">
                                                <button class="btn btn-danger btn-sm" onclick="removeInstructor({{ $instructor->id }})">
                                                    <i class="fas fa-trash"></i>
                                                </button>
                                            </div>
                                        </div>
                    ...
                    @else
                        <div class="empty-state">
                            ...
                            <h6>No instructors assigned</h6>
                            <p>Add instructors to this course to get started.</p>
                        </div>
                    @endif
                </div>
            </div>
                ...
                <div class="card-body">
                                ...
                                <div class="col-md-6 col-lg-4 mb-3">
                                    <div class="list-item">
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div class="flex-grow-1 ms-3">
                                                <h6 class="mb-1">{{ $material->title ?? 'N/A' }}</h6>
                                                <p class="mb-1 text-muted small">{{ Str::limit($material->description ?? '', 50) }}</p>
                                                <span class="badge bg-success">{{ $material->type ?? 'Document' }}</span>
                                            </div>
                                            <div class="flex-shrink-0">
                                                <a href="{{ $material->file_url ?? '#' }}" class="btn btn-info btn-sm me-1" target="_blank">
                    ...
                    @else
                        <div class="empty-state">
                            ...
                            <p>Add materials to this course to help students learn.</p>
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="card info-card mb-4">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <h5 class="section-title mb-0">
                            <i class="fas fa-file-alt me-2"></i>Course Exams
                        </h5>
                                ...
                                <div class="col-md-6 col-lg-4 mb-3">
                                    <div class="list-item">
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div class="flex-grow-1 ms-3">
                                                <h6 class="mb-1">{{ $exam->title ?? 'N/A' }}</h6>
                                                <p class="mb-1 text-muted small">{{ Str::limit($exam->description ?? '', 50) }}</p>
                                                <span class="badge bg-warning">{{ $exam->type ?? 'Quiz' }}</span>
                                            </div>
                                            <div class="flex-shrink-0">
                                                <a href="#" class="btn btn-info btn-sm me-1">
                                                    <i class="fas fa-eye"></i>
                                                </a>
                                                <button class="btn btn-danger btn-sm" onclick="removeExam({{ $exam->id }})">
                                                    <i class="fas fa-trash"></i>
                                                </button>
                                            </div>
                                        </div>
                    ...
                    @else
                        <div class="empty-state">
                            ...
                            <p>Add exams to this course to assess student progress.</p>
                        </div>
                    @endif
                </div>
            </div>
    ...
    function removeInstructor(instructorId) {
        if (confirm('Are you sure you want to remove this instructor?')) {
            fetch(`/admin/course/instructors/${instructorId}`, {
                method: 'DELETE',
                headers: {
    ...
    function removeMaterial(materialId) {
        if (confirm('Are you sure you want to remove this material?')) {
            fetch(`/admin/course/materials/${materialId}`, {
                method: 'DELETE',
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                    'Content-Type': 'application/json',
    ...
    function removeExam(examId) {
        if (confirm('Are you sure you want to remove this exam?')) {
            fetch(`/admin/course/exams/${examId}`, {
                method: 'DELETE',
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                    'Content-Type': 'application/json',
